<!-- About Section -->
    <section class="success" id="about">
        <div class="container">
            <div class="row">
                <div class="col-lg-12 text-center">
                    <h2>About</h2>
                    <hr class="star-light">
                </div>
            </div>
            <div class="row">
                <div class="col-lg-4 col-lg-offset-2">
                    <p>We care about your college essays and want you to be able to put forth your best work. That's why we assign only the best and most professional editors to revise your essay.</p>
                </div>
                <div class="col-lg-4">
                    <p>We edit every college essay to your needs. You will be able to directly email the editors and ask questions or raise concerns, hassle-free.</p>
                </div>
            </div>
        </div>
    </section>

    <section class="howitworks" id="howitworks">
        <div class="container">
            <div class="row">
                <div class="col-lg-12 text-center">
                    <h2>How It Works</h2>
                    <hr class="star-primary">
                </div>
            </div>
            <div class="row">
                <div class="col-lg-4 col-lg-offset-2">
                    <p>You upload your college essay and our highly qualified editors will give you comprehensive feedback regarding grammar, sentence flow, diction, structure, tone, and essay topic.</p>
                </div>
                <div class="col-lg-4">
                    <p>Our editors are also open to any questions about any edits they make. They will also answer short questions about future drafts, and they will answer any general questions about essay writing.</p>
                </div>
            </div>
        </div>
    </section>

    <section class="success" id="rewards">
        <div class="container">
            <div class="row">
                <div class="col-lg-12 text-center">
                    <h2>Rewards</h2>
                    <hr class="star-light">
                </div>
            </div>
            <div class="row">
                <div class="col-lg-6 col-lg-offset-3">
                    <p>Have someone refer you! If someone supplies your email when they purchase our editing service, 
                    you will get 10% in rewards! In other words, every 500 word essay someone purchases with your email as a referral,
                    you get $5. Spread the word!</p>
                </div>
            </div>
        </div>
    </section>

    

    <!-- Footer -->
    <footer class="text-center">
        <div class="footer-above">
            <div class="container">
                <div class="row">
                    <div class="footer-col col-md-4">
                        <h3>No Rush Period</h3>
                    </div>
                    <div class="footer-col col-md-4">
                        <h3>Feedback within 24 hours</h3>
                    </div>
                    <div class="footer-col col-md-4">
                        <h3>Half the price of other services</h3>
                    </div>
                </div>
                <div class="row" style="margin-bottom:15px">Questions or Concerns? Email us at scwediting@gmail.com!</div>
            </div>
        </div>
        <div class="footer-below">
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
                        Copyright &copy; SCW Revision
                    </div>
                </div>
            </div>
        </div>
    </footer>

    <!-- Scroll to Top Button (Only visible on small and extra-small screen sizes) -->
    <div class="scroll-top page-scroll visible-xs visible-sm">
        <a class="btn btn-primary" href="#page-top">
            <i class="fa fa-chevron-up"></i>
        </a>
    </div>